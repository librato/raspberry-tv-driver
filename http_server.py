...
class ServerHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        process = subprocess.call(["/usr/bin/killall", "-9", "iceweasel"])
        #start browser with url
        web_command = ['iceweasel','--display=:0','--new-window',url]
        print "Running > ", web_command, " < as a sub process"
        subprocess.Popen(web_command )
        print "zzzzzzz....."
        time.sleep(20)
        print "Robot is hitting the F11 key"
        subprocess.Popen('WID=`xdotool search --class iceweasel`; xdotool windowactivate $WID; xdotool key --clearmodifiers F11',shell=True)
